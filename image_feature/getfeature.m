...
subplot(2,2,2),imshow(H,[]),title('H分量图');
subplot(2,2,3),imshow(S,[]),title('S分量图');
subplot(2,2,4),imshow(V,[]),title('V分量图');
figure,imshow(I_H);title('H-S直方图');
...
% figure;
% subplot(131);imhist(R);title('Red分量直方图');
% subplot(132);imhist(G);title('Green分量直方图');
% subplot(133);imhist(B);title('Blue分量直方图');
% figure;
% Histcounts=REDcounts+Greencounts+Bluecounts;
% Histogramdata=Histcounts';
% plot(Histogramdata);title('RGB颜色直方图');
